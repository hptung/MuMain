...
void CNewUITextInputMsgBox::ValidateNumber()
{
	if (!m_bUseLimit)
		return;
	...
	if (value < m_MinNumber)
	{
		value = m_MinNumber;

		wchar_t newText[32];
		swprintf(newText, 32, L"%d", value);

		m_pInputBox->SetText(newText);
	}
	if (value > m_MaxNumber)
	{
		value = m_MaxNumber;
...
bool SEASON3B::CEnterStatPointValueMsgBoxLayout::SetLayout()
{
	CNewUITextInputMsgBox* pMsgBox = GetMsgBox();
	if (0 == pMsgBox)
		return false;
	...
	pMsgBox->SetInputBoxOption(UIOPTION_NUMBERONLY | UIOPTION_PAINTBACK);
	pMsgBox->AddMsg(GlobalText[2326]);
	pMsgBox->EnableNumberLimit(true);
	pMsgBox->SetMinNumber(1);
	pMsgBox->SetMaxNumber(CharacterAttribute->LevelUpPoint);
	pMsgBox->AddCallbackFunc(CEnterStatPointValueMsgBoxLayout::ReturnDown, MSGBOX_EVENT_PRESSKEY_RETURN);
	pMsgBox->AddCallbackFunc(CEnterStatPointValueMsgBoxLayout::OkBtnDown, MSGBOX_EVENT_USER_COMMON_OK);
	pMsgBox->AddCallbackFunc(CEnterStatPointValueMsgBoxLayout::CancelBtnDown, MSGBOX_EVENT_USER_COMMON_CANCEL);

	return true;
}

int SEASON3B::CEnterStatPointValueMsgBoxLayout::m_StatType = -1;

CALLBACK_RESULT SEASON3B::CEnterStatPointValueMsgBoxLayout::ProcessOk(class CNewUIMessageBoxBase* pOwner, const leaf::xstreambuf& xParam)
{
	auto* pMsgBox = dynamic_cast<CNewUITextInputMsgBox*>(pOwner);
	...
	// 👉 giới hạn theo point còn lại
	if (value > CharacterAttribute->LevelUpPoint)
		value = CharacterAttribute->LevelUpPoint;

	// =========================
	// 🔥 CÁCH 1: gửi loop (không sửa server)
	// =========================
	for (int k = 0; k < value; k++)
	{
		SocketClient->ToGameServer()->SendIncreaseCharacterStatPoint(
			(CharacterStatAttribute)m_StatType
		);
	}

	// =========================
	// 🔥 CÁCH 2: nếu bạn đã sửa server
	// =========================
	/*
	SocketClient->ToGameServer()->SendIncreaseCharacterStatPointEx(
		(CharacterStatAttribute)g_SelectedStatType,
		value
	);
	*/
...
CALLBACK_RESULT SEASON3B::CEnterStatPointValueMsgBoxLayout::ReturnDown(class CNewUIMessageBoxBase* pOwner, const leaf::xstreambuf& xParam)
{
	return ProcessOk(pOwner, xParam);
}

CALLBACK_RESULT SEASON3B::CEnterStatPointValueMsgBoxLayout::OkBtnDown(class CNewUIMessageBoxBase* pOwner, const leaf::xstreambuf& xParam)
{
	return ProcessOk(pOwner, xParam);
}

CALLBACK_RESULT SEASON3B::CEnterStatPointValueMsgBoxLayout::CancelBtnDown(class CNewUIMessageBoxBase* pOwner, const leaf::xstreambuf& xParam)
{
	PlayBuffer(SOUND_CLICK01);
